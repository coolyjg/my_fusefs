...
// mod ffi_wrapper;
include!("ffi_wrapper.rs");
// use crate::ffi_wrapper::PMEMobjpool;
// pub static mut my_pop: Option<*mut PMEMobjpool>= None;
...
impl LockedINode{
    ...
    pub fn read_at(&self, offset: usize, buf: &mut [u8])->Result<usize>{
        ...
        let len = unsafe{
            my_read_at_content(my_pop.unwrap(), ino, offset, buf.as_ptr())
        };
        Ok(len as usize)
    }
    pub fn write_at(&self, offset: usize, buf: &[u8]) -> Result<usize> {
        ...
        let ino = file.extra.ino as u32;
        let len = unsafe{
            my_write_at_content(my_pop.unwrap(), ino, offset, buf.as_ptr(), size)
        };
        Ok(len as usize)
    }
